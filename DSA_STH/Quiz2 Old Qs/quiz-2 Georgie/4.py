class BinaryTree:
    ...
    def set_level(self, root, level=0):
        if root is not None:
            try:
                level_dict[level].append(root.data)
            except:
                level_dict[level] = [root.data]
            self.set_level(root.left, level + 1)
            self.set_level(root.right, level + 1)
...
tree.set_level(tree.root)
print(level_dict)
for level,list_node in level_dict.items():
    print(f"Level {level} : {'->'.join(map(str,list_node))}")